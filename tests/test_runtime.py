...
def run_report(agent):
    return json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))


def add_guard_fixture_dirs(tmp_path):
    (tmp_path / "pure" / "core").mkdir(parents=True)
    (tmp_path / "pure" / "core" / "runtime.py").write_text("runtime\n", encoding="utf-8")
    (tmp_path / "pure" / "README.md").write_text("pure\n", encoding="utf-8")
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".pure").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".pure" not in result
    assert ".git" not in result
    assert "[F] hello.txt" in result


def test_tool_repetition_guard_warns_on_consecutive_same_list_files_call(tmp_path):
    add_guard_fixture_dirs(tmp_path)
    agent = build_agent(
        tmp_path,
        [
            '<tool>{"name":"list_files","args":{"path":"pure"}}</tool>',
            '<tool>{"name":"list_files","args":{"path":"pure"}}</tool>',
            "<final>Done.</final>",
        ],
    )

    assert agent.ask("Inspect pure") == "Done."

    events = trace_events(agent)
    repeated_events = [event for event in events if event["event_type"] == "repeated_tool_call_detected"]
    assert len(repeated_events) == 1
    assert repeated_events[0]["tool_name"] == "list_files"
    assert repeated_events[0]["previous_step"] == 1
    assert repeated_events[0]["current_step"] == 2
    assert repeated_events[0]["window"] == 5
    assert repeated_events[0]["mode"] == "warn"
    assert run_report(agent)["repeated_tool_call_count"] == 1


def test_tool_repetition_guard_normalizes_windows_style_paths(tmp_path):
    add_guard_fixture_dirs(tmp_path)
    agent = build_agent(
        tmp_path,
        [
    ...
    agent.ask("Inspect pure twice")

    repeated_event = [
        event for event in trace_events(agent) if event["event_type"] == "repeated_tool_call_detected"
    ][0]
    assert repeated_event["normalized_args"] == '{"path":"pure"}'
    tool_history = [item for item in agent.session["history"] if item["role"] == "tool"]
    assert "warning: repeated tool call detected" in tool_history[-1]["content"]


def test_tool_repetition_guard_does_not_warn_for_different_args(tmp_path):
    add_guard_fixture_dirs(tmp_path)
    agent = build_agent(
        tmp_path,
        [
            '<tool>{"name":"list_files","args":{"path":"pure"}}</tool>',
            '<tool>{"name":"list_files","args":{"path":"pure/core"}}</tool>',
            "<final>Done.</final>",
        ],
    )

    agent.ask("Inspect two dirs")

    assert [event["event_type"] for event in trace_events(agent)].count("repeated_tool_call_detected") == 0
    assert run_report(agent)["repeated_tool_call_count"] == 0


def test_tool_repetition_guard_allows_call_after_window(tmp_path):
    add_guard_fixture_dirs(tmp_path)
    agent = build_agent(
        tmp_path,
        [
        ...
        tool_repetition_guard={"window": 1},
    )

    agent.ask("Inspect with a narrow repetition window")

    assert [event["event_type"] for event in trace_events(agent)].count("repeated_tool_call_detected") == 0
    assert run_report(agent)["repeated_tool_call_count"] == 0


def test_tool_repetition_guard_warn_mode_does_not_block_handler(tmp_path):
    add_guard_fixture_dirs(tmp_path)
    agent = build_agent(
        tmp_path,
        [
        ...
        tool_repetition_guard={"mode": "warn"},
    )
    calls = []

    def fake_list_files(args):
        calls.append(dict(args))
        return "listed"

    agent.tools["list_files"]["run"] = fake_list_files

    agent.ask("Inspect pure twice")

    assert calls == [{"path": "pure"}, {"path": "pure"}]
    tool_history = [item for item in agent.session["history"] if item["role"] == "tool"]
    assert tool_history[-1]["content"].startswith("warning: repeated tool call detected")
    assert "listed" in tool_history[-1]["content"]


def test_tool_repetition_guard_block_mode_rejects_without_calling_handler(tmp_path):
    add_guard_fixture_dirs(tmp_path)
    agent = build_agent(
        tmp_path,
        [
        ...
        tool_repetition_guard={"mode": "block"},
    )
    calls = []
    ...
    assert calls == [{"path": "pure"}]
    events = trace_events(agent)
    rejected_events = [event for event in events if event["event_type"] == "tool_rejected_repeated_call"]
    assert len(rejected_events) == 1
    assert rejected_events[0]["tool_name"] == "list_files"
    tool_history = [item for item in agent.session["history"] if item["role"] == "tool"]
    assert tool_history[-1]["content"].startswith("error: repeated tool call rejected")
    assert run_report(agent)["repeated_tool_call_count"] == 1


def test_tool_repetition_guard_block_mode_still_allows_different_args(tmp_path):
    add_guard_fixture_dirs(tmp_path)
    agent = build_agent(
        tmp_path,
        [
    ...
    agent.ask("Inspect pure and pure/core")

    assert calls == [{"path": "pure"}, {"path": "pure/core"}]
    assert run_report(agent)["repeated_tool_call_count"] == 1


def test_tool_repetition_guard_ignores_non_semantic_args(tmp_path):
    add_guard_fixture_dirs(tmp_path)
    agent = build_agent(
        tmp_path,
        [
            '<tool>{"name":"list_files","args":{"path":"pure","display_limit":10}}</tool>',
            '<tool>{"name":"list_files","args":{"display_limit":20,"path":"pure"}}</tool>',
            "<final>Done.</final>",
        ],
    )

    agent.ask("Inspect pure with display limits")
...
def test_tool_repetition_guard_can_be_disabled(tmp_path):
    add_guard_fixture_dirs(tmp_path)
    agent = build_agent(
        tmp_path,
        [
        ...
        tool_repetition_guard={"enabled": False},
    )
...
def test_tool_repetition_guard_invalid_mode_falls_back_to_warn(tmp_path):
    add_guard_fixture_dirs(tmp_path)
    agent = build_agent(
        tmp_path,
        [
        ...
        tool_repetition_guard={"mode": "reject"},
    )
    ...
    report = run_report(agent)
    assert repeated_event["mode"] == "warn"
    assert report["repeated_tool_call_count"] == 1
    assert report["runtime_config_warnings"]
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "pure/0.1"
...
def test_anthropic_compatible_client_posts_expected_messages_payload():
    ...
    client = AnthropicCompatibleModelClient(
        ...
        base_url="https://www.api.anthropic.example/v1",
        api_key="sk-test",
        temperature=0.2,
        timeout=30,
    )
    ...
    assert result == "<final>ok</final>"
    assert captured["url"] == "https://www.api.anthropic.example/v1/messages"
    assert captured["timeout"] == 30
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        os.environ,
        {
            "OPENAI_API_BASE": "https://www.api.example.test/v1",
        ...
        with patch(
            "pure.cli.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("pure.cli.cli.OpenAICompatibleModelClient") as mock_openai:
    ...
    assert mock_openai.call_args.kwargs["base_url"] == "https://www.api.example.test/v1"
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "pure.cli.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("pure.cli.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("pure.cli.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_deepseek_provider_and_env_configuration(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "PURE_DEEPSEEK_API_BASE=https://api.deepseek.com/anthropic",
                "PURE_DEEPSEEK_API_KEY=sk-project-deepseek",
                "PURE_DEEPSEEK_MODEL=deepseek-v4-pro",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def test_trace_and_report_redact_secret_env_values(tmp_path):
    ...
    patch_env = {"OPENAI_API_KEY": secret}
    if os.name == "nt":
        patch_env["ComSpec"] = os.environ.get("ComSpec", "")
        patch_env["SystemRoot"] = os.environ.get("SystemRoot", "")
        if not patch_env["ComSpec"] or not patch_env["SystemRoot"]:
            raise pytest.skip("Windows shell env vars ComSpec/SystemRoot are unavailable")
    with patch.dict(os.environ, patch_env, clear=True):
        shell_env_allowlist = None
        if os.name == "nt":
            # On Windows, subprocess(shell=True) requires ComSpec/SystemRoot to locate cmd.exe.
            shell_env_allowlist = tuple(MiniAgent(model_client=FakeModelClient([]), workspace=build_workspace(tmp_path), session_store=SessionStore(tmp_path / ".pure" / "sessions")).shell_env_allowlist) + (
                "ComSpec",
                "SystemRoot",
            )
        agent = build_agent(
    ...
    assert "<redacted>" in tool_events[0]["result"] or "exit_code" in tool_events[0]["result"]
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .pure/.\n"
    ...
    index_path = tmp_path / ".pure" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".pure" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".pure" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .pure/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".pure" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".pure" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(mini_pkg.__file__).as_posix().endswith("/pure/__init__.py")
