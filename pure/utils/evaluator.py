...
from ..core.models import FakeModelClient
...
class BenchmarkEvaluator:
    def __init__(
        ...
        self.workspace_root = Path(workspace_root) if workspace_root is not None else Path(
            tempfile.mkdtemp(prefix="pure-benchmark-")
        )
    ...
    def run_task(self, task):
        ...
        session_store = SessionStore(fixture_copy_root / ".pure" / "sessions")
        run_store = RunStore(fixture_copy_root / ".pure" / "runs")
