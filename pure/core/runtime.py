...
PureRuntime is the control loop around the model: it builds prompts, parses model output,
...
from ..services import CheckpointService, MemoryService, PromptService, ToolExecutionService, TraceService, WorkspaceService
from ..services.tool_repetition_guard import ToolRepetitionGuardConfig
...
class PureRuntime:
    ...
    CHECKPOINT_NONE_STATUS = CHECKPOINT_NONE_STATUS
    CHECKPOINT_FULL_VALID_STATUS = CHECKPOINT_FULL_VALID_STATUS
    CHECKPOINT_PARTIAL_STALE_STATUS = CHECKPOINT_PARTIAL_STALE_STATUS
    CHECKPOINT_WORKSPACE_MISMATCH_STATUS = CHECKPOINT_WORKSPACE_MISMATCH_STATUS
    CHECKPOINT_SCHEMA_MISMATCH_STATUS = CHECKPOINT_SCHEMA_MISMATCH_STATUS
    ...
    def __init__(
        ...
        self.tool_repetition_guard_config = ToolRepetitionGuardConfig.from_mapping(tool_repetition_guard)
        self.runtime_config_warnings = list(self.tool_repetition_guard_config.warnings)
        self.recent_tool_calls = []
        self.repeated_tool_call_count = 0
        self._pending_startup_trace_events = []
        self.mcp_registry = None
        self.run_store = run_store or RunStore(Path(workspace.repo_root) / ".pure" / "runs")
        ...
        self.configure_mcp_tools(mcp_client_factory=mcp_client_factory)
        ...
        self.tool_execution_service = ToolExecutionService(self)
        self.trace_service = TraceService(self.run_store)
        self.knowledge_service = KnowledgeService(self.root)
        self.knowledge_context = "Knowledge context:\n- none"
    ...
    def invalidate_stale_memory(self):
        return self.memory_service.invalidate_stale_memory()

    def evaluate_resume_state(self):
        return self.checkpoint_service.evaluate_resume_state()
    ...
    def configure_mcp_tools(self, mcp_client_factory=None):
        mcp_config = dict(self.runtime_config.get("mcp") or {})
        if not bool(mcp_config.get("enabled", False)):
            return []
        try:
            from ..integrations.mcp_client.registry import MCPClientRegistry

            self.mcp_registry = MCPClientRegistry(client_factory=mcp_client_factory)
            events = self.mcp_registry.register_tools(self, mcp_config)
            self._pending_startup_trace_events.extend(events)
            return events
        except Exception as exc:
            self.runtime_config_warnings.append(f"mcp registration failed: {exc}")
            return []
    ...
    def build_prefix(self):
        ...
        text = textwrap.dedent(
            f"""\
            You are Pure, an agent runtime for long-lived technical tasks working inside a local repository.
    ...
    def refresh_prefix(self, force=False):
        return self.workspace_service.refresh_prefix(force=force)
    ...
    def memory_text(self):
        return self.memory_service.memory_text()
    ...
    def _build_prompt_and_metadata(self, user_message):
        return self.prompt_service.build_prompt_and_metadata(user_message)
    ...
    def retrieve_knowledge_context(self, user_message, budget_chars=1400, top_k=5):
        try:
            if not getattr(self.knowledge_service.vector_store, "records", []):
                self.knowledge_service.index_project(reset=True)
            context, sources = self.knowledge_service.retrieve_for_context(
                user_message,
                top_k=top_k,
            ...
            sources = [{"source": "knowledge", "score": 0.0, "metadata": {"error": str(exc)}, "content_chars": 0}]
    ...
    def emit_trace(self, task_state, event, payload=None):
        ...
        trace_event = self.trace_service.append(task_state, event, payload)
        self._last_trace_event = trace_event
        return trace_event
        # trace 是运行中的逐事件时间线，适合回答“这一轮 agent 到底做了什么”。
        return self.trace_service.append(task_state, event, payload)

    def flush_startup_trace_events(self, task_state):
        events = list(getattr(self, "_pending_startup_trace_events", []) or [])
        self._pending_startup_trace_events = []
        for event_type, payload in events:
            self.emit_trace(task_state, event_type, payload)
    ...
    def create_checkpoint(self, task_state, user_message, trigger):
        return self.checkpoint_service.create_checkpoint(task_state, user_message, trigger)
        state = self.checkpoint_state()
    ...
    def update_memory_after_tool(self, name, args, result):
        return self.memory_service.update_after_tool(name, args, result)
    ...
    def note_tool(self, name, args, result):
        return self.memory_service.note_tool(name, args, result)
    ...
    def ask(self, user_message, task_id=None, run_id=None):
        ...
        if self.runtime_config_warnings:
            run_started_payload["runtime_config_warnings"] = list(self.runtime_config_warnings)
        self.emit_trace(task_state, "run_started", run_started_payload)
        self.flush_startup_trace_events(task_state)
        knowledge_started_at = time.monotonic()
        _, knowledge_sources = self.retrieve_knowledge_context(user_message)
        self.emit_trace(
            task_state,
            "knowledge_retrieved",
            {
                ...
                "duration_ms": int((time.monotonic() - knowledge_started_at) * 1000),
            },
        )
        ...
        while tool_steps < self.max_steps and attempts < max_attempts:
            ...
            if kind == "tool":
                ...
                self.emit_trace(
                    ...
                    {
                        "name": name,
                        "tool_status": dict(self._last_tool_result_metadata or {}).get("tool_status", "ok"),
                        "tool_error_code": dict(self._last_tool_result_metadata or {}).get("tool_error_code", ""),
                    },
                )
                self.record(
                    {
    ...
    def run_tool(self, name, args):
        return self.tool_execution_service.run_tool(name, args)
    ...
    def repeated_tool_call(self, name, args):
        ...
        # 这里提前挡掉最简单的这种循环。
        return self.tool_execution_service.repetition_guard.check(name, args) is not None
    ...
    def build_report(self, task_state):
        ...
        return {
            ...
            "knowledge_sources": list(getattr(self, "knowledge_sources", []) or []),
            "repeated_tool_call_count": int(getattr(self, "repeated_tool_call_count", 0) or 0),
            "runtime_config_warnings": list(getattr(self, "runtime_config_warnings", []) or []),
            "durable_promotions": list(self.last_durable_promotions),
            "durable_rejections": list(self.last_durable_rejections),
            "durable_superseded": list(self.last_durable_superseded),
    ...
    def parse(raw):
        ...
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            body = PureRuntime.extract(raw, "tool")
            try:
                payload = json.loads(body)
            except Exception:
                return "retry", PureRuntime.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", PureRuntime.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", PureRuntime.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", PureRuntime.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = PureRuntime.parse_xml_tool(raw)
            if payload is not None:
                return "tool", payload
            return "retry", PureRuntime.retry_notice()
        if "<final>" in raw:
            final = PureRuntime.extract(raw, "final").strip()
            if final:
                return "final", final
            return "retry", PureRuntime.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if raw:
            return "final", raw
        return "retry", PureRuntime.retry_notice("model returned an empty response")
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = PureRuntime.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = PureRuntime.extract_raw(body, key)
...
Pico = PureRuntime  # backward-compatible alias
