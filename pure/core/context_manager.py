...
DEFAULT_REDUCTION_ORDER = ("knowledge_context", "relevant_memory", "history", "memory", "prefix")
SECTION_ORDER = ("prefix", "memory", "knowledge_context", "relevant_memory", "history", "current_request")
...
class ContextManager:
    ...
    def build(self, user_message):
        ...
        在 agent 链路里的位置：
        它位于 `PureRuntime.ask()` 的每轮模型调用之前，是“真正发请求给模型”
        ...
        section_texts = {
            ...
            "knowledge_context": self._knowledge_context_text(),
            "history": "",
        ...
        while len(prompt) > self.total_budget:
            ...
            for section in self.reduction_order:
                ...
                if rendered.get(section) and rendered[section].raw_chars <= current_budget:
                    continue
    ...
    def _render_sections_without_reduction(self, section_texts, selected_notes=None):
        ...
        return {
            ...
            "knowledge_context": SectionRender(
                raw=section_texts["knowledge_context"],
                budget=len(section_texts["knowledge_context"]),
                rendered=section_texts["knowledge_context"],
                details=self._knowledge_context_details(),
            ),
    ...
    def _render_sections(self, section_texts, budgets, selected_notes=None):
        ...
        for section in SECTION_ORDER:
            ...
            elif section == "knowledge_context":
                raw = section_texts[section]
                rendered_text = _tail_clip(raw, int(budget or 0))
    ...
    def _assemble_prompt(self, rendered):
        ...
        return "\n\n".join(
            [
                ...
                rendered["knowledge_context"].rendered,
    ...
    def _metadata(self, prompt, rendered, budgets, reduction_log, selected_notes, user_message, section_texts):
        ...
        return {
            ...
            "knowledge_context": {
                "selected_count": len(getattr(self.agent, "knowledge_sources", []) or []),
                "sources": list(getattr(self.agent, "knowledge_sources", []) or []),
                "raw_chars": rendered["knowledge_context"].raw_chars,
                "rendered_chars": rendered["knowledge_context"].rendered_chars,
                "budget_chars": int(budgets.get("knowledge_context", 0)),
            },
            "history": {
    ...
    def _knowledge_context_text(self):
        text = str(getattr(self.agent, "knowledge_context", "") or "").strip()
        return text or "Knowledge context:\n- none"

    def _knowledge_context_details(self):
        return {"sources": list(getattr(self.agent, "knowledge_sources", []) or [])}
